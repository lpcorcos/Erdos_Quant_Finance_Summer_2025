...
def mc_Euro(S, T, sigma, steps, K, N, r=0, q=0):
    ...
    This functions option prices using a Monte Carlo simulation approach. It generates
...
def standard_error(payoffs, T, r=0):
    ...
    sigma = np.sqrt(np.sum((payoffs - payoff*np.exp(-r * T))**2) / (N - 1))
...
def black_scholes_simulated_profits(S0, T, K0, K1, sigma, call_sold_premium, call_bought_premium):
    ...
    call_sold_values = np.maximum(stock_paths[-1]-K0, 0)
    ...
    deltas_sold = [MC_delta(avg_stock_values[i], TTE[i], sigma, 10000, K0, 1, 0) for i in range(len(stock_paths))]
    call_deltas_sold = [deltas_sold[i]['delta_call'] for i in range(len(stock_paths))]

    deltas_bought = [MC_delta(avg_stock_values[i], TTE[i], sigma, 10000, K1, 1, 0) for i in range(len(stock_paths))]
    ...
    hedge_call_sold_values = call_sold_values[-1] + call_hedge_sold
    hedge_call_bought_values = call_bought_values[-1] + call_hedge_bought

    position_values = (-call_sold_values+call_bought_values)
    expected_profit = np.mean(position_values) + call_sold_premium - call_bought_premium
    #stand_err = standard_error(position_values, T)
    profit_chance = per_in_money_paths(position_values + call_sold_premium - call_bought_premium)
    ...
    plt.hist(position_values + call_sold_premium - call_bought_premium, bins = 20, color = 'black')
...
def spread_call_profit_loss_graph(historical_stock_price, strike_price_1, num_calls_1, premium_1, strike_price_2, num_calls_2, premium_2):

    spot_prices = np.linspace(.5*historical_stock_price, 2*historical_stock_price, 300)
    ...
    plt.axvline(x=historical_stock_price, label = 'Historical stock price')
    ...
    print(investment, strike_price_1,strike_price_2)
...
def earliest_date(date1: str, date2: str) -> str:
    """
    Returns the earliest date of the two provided dates.
    ...
    Returns:
    str: The earliest date in 'YYYY-MM-DD' format.
    """
    date_format = '%Y-%m-%d'
    ...
    try:
        ...
        return earliest.strftime(date_format)
    except ValueError as e:
        raise ValueError(f"Invalid date format: {e}")
